...
import { Coach, GM, League, Marketing, Owner, Scout, SimCHL, SimPHL } from "../../_constants/constants";
...
const ProfileTeamCard: React.FC<ProfileTeamCardProps> = ({teamID, teamLabel, conference, league, removeUser, isRetro, backgroundColor, borderColor, secondaryBorderColor, role, textColorClass}) => {
    ...
    return <>
        <Border classes="w-full" styles={{ backgroundColor, borderColor }}>
          <div className="flex flex-row flex-grow h-[12rem] w-full">
            <Border styles={{borderColor: secondaryBorderColor}}
...
const {chlTeam, removeUserfromCHLTeamCall} = useSimHCKStore();
  ...
  const secondaryBorderColor = chlTeam?.ColorThree || "#4B5563";
        ...
        />
            ...
            <ButtonGroup>
                ...
                <Text className="mb-4 text-start">
                    Warning: By resigning, you will lose access to the team in this league. Are you sure you would like to resign?
                </Text>
        </ProfileTeamCardModal>
        </>}
    </>

  ...
  return (
    <>
      <div className="flex gap-5">
        <SectionCards team={team} header="Team News" classes={`${textColorClass}`}>
          {isLoading ? (
            <div className="flex justify-center items-center">
              <Text variant="small" classes={`${textColorClass}`}>Loading...</Text>
            </div>
          ) : (
            filteredCollegeNews.map((news, index) => (
              <div key={index} className="mb-2">
                <Text variant="small" classes={`${textColorClass}`}>{news.Message}</Text>
              </div>
            ))
          )}
        </SectionCards>
        <SectionCards team={team} header="Inbox" classes={`${textColorClass}`}>
          {isLoading ? (
            ...
            filteredNotifications.map((notification, index) => (
              <div key={index} className="mb-2">
                <Text variant="small" classes={`${textColorClass}`}>{notification.Subject}</Text>
                <Text variant="small" classes={`${textColorClass}`}>{notification.Message}</Text>
              </div>
            ))
          ) : (
            <Text variant="small" classes={`${textColorClass}`}>Your Inbox is Empty</Text>
          )}
        </SectionCards>
      </div>
    </>
